...
const FinalizeOrder = () => {
  ...
  // Fetch order details using the session_id
  useEffect(() => {
        ...
        <div className="main-box border border-gray-200 rounded-xl pt-6 max-w-xl max-lg:mx-auto lg:max-w-full">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between px-6 pb-6 border-b border-gray-200">
            <div className="data">
              <p className="font-semibold text-base leading-7 text-black">
                Order Id: <span className="text-indigo-600 font-medium">#{orderDetails.orderId}</span>
              </p>
              <p className="font-semibold text-base leading-7 text-black mt-4">
                Order Payment: <span className="text-gray-400 font-medium">{orderDetails.paymentDate}</span>
              </p>
            </div>
          ...
          <div className="w-full border-t border-gray-200 px-6 flex flex-col lg:flex-row items-center justify-between">
            ...
            <p className="font-semibold text-lg text-black py-6">
              Total Price: <span className="text-indigo-600">${orderDetails.totalPrice}</span>
            </p>
          </div>
          <button onClick={handlePlaceOrder} className="w-full bg-indigo-600 py-3 text-white text-lg rounded-full mt-5 hover:bg-indigo-700">
            Confirm Order
          </button>
        </div>
      </div>
